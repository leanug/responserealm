...
const useLikedPostActions = () => {
  ...
  const handleLike = async (postId: string) => {
    ...
    const url = `${ENV.BASE_URL}/${ENV.ENDPOINTS.LIKED_POST.CREATE}`
    const data = {
      postId
    }
    ...
    if (response.ok) {
      ...
      const updateLikesUrl = `
        ${ENV.BASE_URL}/${ENV.ENDPOINTS.POST.LIKE(postId, 'like')}
      `
  ...
  const handleDislike = async (postId: string, likedPostId: string) => {    
    ...
    const url = `
      ${ENV.BASE_URL}/${ENV.ENDPOINTS.LIKED_POST.DELETE(likedPostId)}
    `
    ...
    if (response.ok) {
      ...
      const updateLikesUrl = `
        ${ENV.BASE_URL}/${ENV.ENDPOINTS.POST.LIKE(postId, 'dislike')}
      `
      const updateLikesResponse = await fetch(updateLikesUrl, { 
        method: 'PATCH'
      })
